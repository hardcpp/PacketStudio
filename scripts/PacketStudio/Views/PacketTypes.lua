...
PacketStudio.Views.PacketTypes = {}
PacketStudio.Views.PacketTypes.Visible = false
...
-- Render function
-- =======================================================
function PacketStudio_PacketTypes_MainDraw()
    if PacketStudio.Views.PacketTypes.Visible == false then
        return
    end

    local l_MainWindow;
    _, l_MainWindow = ImGui.Begin("PacketStudio::PacketTypes", true, ImVec2(600, 400), -1.0)
    if l_MainWindow then
        ...
        if ImGui.CollapsingHeader("Results", "id_packet_types_result", true) then
            ...
            for l_Key,l_Value in pairs(PacketStudio.PacketTypes) do
                ...
                if g_PacketTypeFilter:len() ~= 0 then
                    ...
                    local l_MatchB = string.find(l_DirectionStr, g_PacketTypeFilter:lower()) and true or false
                    local l_MatchC = string.find(l_Value[3]:lower(), g_PacketTypeFilter:lower()) and true or false

                    if not l_MatchA and not l_MatchB and not l_MatchC then
...
Pyx.Scripting.CurrentScript:RegisterCallback("ImGui.OnRender", PacketStudio_PacketTypes_MainDraw)